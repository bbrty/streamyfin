...
var AsyncRateLimiter = class {
	...
	constructor(fn, initialOptions) {
		this.fn = fn;
		this.store = new _tanstack_store.Store(getDefaultAsyncRateLimiterState());
		...
		if (this.key) require_event_client.pacerEventClient.on("d-AsyncRateLimiter", (event) => {
	...
	#setState = (newState) => {
		...
		require_event_client.emitChange("AsyncRateLimiter", this);
	};
	/**
	...
	*/
	#getLimit = () => {
		return require_utils.parseFunctionOrValue(this.options.limit, this);
	};
	/**
	* Returns the current time window in milliseconds
	*/
	#getWindow = () => {
		return require_utils.parseFunctionOrValue(this.options.window, this);
	};
	#execute = async (...args) => {
...
exports.asyncRateLimiterOptions = asyncRateLimiterOptions;
