...
var AsyncQueuer = class {
	...
	constructor(fn, initialOptions = {}) {
		this.fn = fn;
		this.store = new _tanstack_store.Store(getDefaultAsyncQueuerState());
		...
		this.execute = async (position) => {
			...
			if (item !== void 0) {
				...
				try {
					const currentAsyncRetryer = new require_async_retryer.AsyncRetryer(this.fn, {
		...
		if (this.key) require_event_client.pacerEventClient.on("d-AsyncQueuer", (e) => {
	...
	#setState = (newState) => {
		...
		require_event_client.emitChange("AsyncQueuer", this);
	};
	/**
	...
	*/
	#getWait = () => {
		return require_utils.parseFunctionOrValue(this.options.wait ?? 0, this);
	};
	/**
	...
	*/
	#getConcurrency = () => {
		return require_utils.parseFunctionOrValue(this.options.concurrency ?? 1, this);
	};
	/**
...
exports.asyncQueuerOptions = asyncQueuerOptions;
