...
import type { JobStatus, DownloadedItem } from "../types";
import { generateFilename, uriToFilePath, filePathToUri } from "../utils";
...
export function useDownloadOperations({
  ...
  const handleDownloadSuccess = useCallback(async (processId: string, filePath: string) => {
    ...
    const process = getResumeState(processId)?.jobStatus || processes.find(p => p.id === processId);
    if (!process) return;

    const commitToDatabase = (finalFilePath: string, isTranscoded: boolean) => {
      try {
        ...
        console.log(`[DOWNLOAD] Committed ${process.item.Name} to local database.`);
      } catch (e) {
    ...
    if (process.localTranscodeState?.isLocal) {
      ...
      if (Platform.OS === 'ios') {
        targetVCodec = codec === "libx265" ? "hevc_videotoolbox" : "h264_videotoolbox";
      }
        ...
        FFmpegKit.executeAsync(
          ...
          async (session) => {
            ...
            if (ReturnCode.isSuccess(returnCode)) {
              ...
              try { oldFile.delete(); } catch (e) {}
              
              const transcodedFile = new File(filePathToUri(outputPath));
              try { transcodedFile.move(new File(filePathToUri(filePath))); } catch (e) { console.log('move err', e); }

              commitToDatabase(filePath, true);

              setProcesses(prev => prev.map(p => p.id === processId ? { ...p, status: "completed", progress: 100 } : p));
              removeResumeState(processId);
            } else {
              const logs = await session.getLogsAsString();
              setProcesses(prev => prev.map(p => p.id === processId ? { ...p, status: "error" } : p));
            }
          },
          (statistics) => {
            if (durationMs > 0) {
              const timeInMs = statistics.getTime();
              const progress = Math.max(0, Math.min(Math.round((timeInMs / durationMs) * 100), 100));
              const speed = statistics.getSpeed();
              
              setProcesses(prev => prev.map(p => p.id === processId && p.localTranscodeState ? {
                ...p,
                localTranscodeState: {
                  ...p.localTranscodeState,
                  progress,
      ...
      setProcesses((prev: JobStatus[]) => prev.map((p) => p.id === processId ? { ...p, status: "completed", progress: 100 } : p));
      removeResumeState(processId);
    }
  ...
  const startBackgroundDownload = useCallback(
    async (
      ...
      try {
        ...
        const actualBitrate = isLocalTranscode ? BITRATES.find(b => b.key === "Max")! : maxBitrate;
        ...
        const jobStatus: JobStatus = {
          ...
          localTranscodeState: isLocalTranscode ? {
            isLocal: true,
            targetBitratebps: settings.localTranscodingBitrate.value || 8000000,
        ...
        console.log(`[DOWNLOAD] Starting download for ${item.Name} (Local Transcoding: ${isLocalTranscode})`);
        ...
        if (contentLength === 0) {
          try {
            ...
            const probeResponse = await fetch(finalDownloadUrl, {
              method: "GET",
              headers: authHeader ? { Authorization: authHeader, Range: "bytes=0-0" } : { Range: "bytes=0-0" },
              signal: controller.signal,
            });
            clearTimeout(timeoutId);
        ...
        if (isChunkable) {
          console.log(`[DOWNLOAD] Using JS Chunk-Based downloading for ${processId} (${contentLength} bytes)`);
          ...
          setProcesses((prev) => prev.map((p) => p.id === processId ? updatedJobStatus : p));
          ...
          chunkCoordinator.startLoop(
            ...
            (bytesWritten: number) => {
              ...
              setProcesses((prev: JobStatus[]) => 
                prev.map((p) => p.id === processId ? {
                  ...
                  progress: Math.floor((bytesWritten / contentLength) * 100)
                } : p)
              );
            },
            (filePath: string) => {
            ...
            (errorMsg: string) => {
              ...
              setProcesses((prev: JobStatus[]) => prev.map((p) => p.id === processId ? { ...p, status: "error" } : p));
            }
          );
        } else {
          console.log(`[DOWNLOAD] Transcoding/Unknown length. Using OS background downloader.`);
  ...
  const pauseDownload = useCallback(
    async (id: string) => {
      ...
      if (process) {
        ...
        setProcesses(prev => prev.map(p => p.id === id ? { ...p, status: "paused" } : p));
      }
  ...
  const resumeDownload = useCallback(
    async (id: string) => {
      ...
      // Update UI state
      setProcesses(prev => prev.map(p => p.id === id ? { ...p, status: "resuming" } : p));
      ...
      if (contentLength > 0) {
        ...
        setProcesses(prev => prev.map(p => p.id === id ? { ...p, estimatedTotalSizeBytes: contentLength } : p));
        ...
        chunkCoordinator.startLoop(
          ...
          (errorMsg: string) => {
            console.error("[DOWNLOAD] Chunk loop failed during resume", errorMsg);
            setProcesses((prev: JobStatus[]) => prev.map((p) => p.id === id ? { ...p, status: "error" } : p));
          }
        );
      ...
      setProcesses(prev => prev.map(p => p.id === id ? { ...p, status: "error" } : p));
      toast.error("Could not determine file size to resume download. Please retry from the server.");
  ...
  const retryDownload = useCallback(
    ...
    [processes, setProcesses, taskMapRef, removeProcess, startBackgroundDownload, t],
  );
