  ...
  return (
    <ListGroup title={t("home.settings.downloads.downloads_title")}>
      ...
      {settings.localTranscodingEnabled && (
        <>
          <ListItem title={t("home.settings.downloads.local_transcoding_bitrate")}>
            <BitrateSelector
          ...
          <ListItem title={t("home.settings.downloads.local_transcoding_codec")}>
             <PlatformDropdown
                ...
                trigger={
                  <View className="bg-neutral-900 h-10 rounded-xl border-neutral-800 border px-3 py-2 flex flex-row items-center justify-between min-w-[120px]">
                    <Text>{settings.localTranscodingCodec === "libx264" ? "H.264" : "H.265"}</Text>
                  </View>
                }
                onOptionSelect={(val) => {
