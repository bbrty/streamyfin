...
export const DownloadCard = ({ process, ...props }: DownloadCardProps) => {
  ...
  const sanitizedProgress = useMemo(() => {
    if (isTranscodingLocal && process?.localTranscodeState?.progress !== undefined) {
  ...
  }, [process?.progress, process?.localTranscodeState?.progress, isTranscodingLocal]);
              ...
              <View className='flex flex-row items-center gap-x-2 mt-0.5'>
                ...
                {isTranscodingLocal && (
                  <View className='flex flex-row items-center gap-x-2'>
                    ...
                    {process.localTranscodeState?.timeInMs !== undefined && process.localTranscodeState?.durationMs !== undefined && (
                      <Text className='text-[10px] text-neutral-400 font-mono'>
                        {formatTimeString(process.localTranscodeState.timeInMs)} / {formatTimeString(process.localTranscodeState.durationMs)}
                      </Text>
                    )}
                  </View>
                )}
